   ...
   class C {
      ...
      prepare_values() {
         (this.invalid = this.task.invalid),
            ...
            this.task._start.setHours(0, 0, 0, 0),
            this.task._end.setHours(23, 59, 59, 999),
            this.compute_x(),
            this.compute_y(),
      ...
      setup_click_event() {
         ...
         p.on(this.group, 'mouseenter', (i) => {
            ...
            p.on(this.group, 'mouseleave', () => {
               ...
               clearTimeout(e),
                  this.gantt.options.popup_on === 'hover' &&
                     ((s = (i = this.gantt.popup) == null ? void 0 : i.hide) ==
                        null ||
                        s.call(i)),
                  this.gantt.$container
                     .querySelector(`.highlight-${t}`)
      ...
      compute_expected_progress() {
         (this.expected_progress =
            d.diff(d.today(), this.task._start, 'hour') /
            this.gantt.config.step),
            (this.expected_progress =
   ...
   const b = [
      ...
      I = {
         ...
         popup: (n) => {
            ...
            n.set_details(
               `${t} - ${e} (${n.task.actual_duration} days${
                  n.task.ignored_duration
                     ? ' + ' + n.task.ignored_duration + ' excluded'
                     : ''
               })<br/>Progress: ${Math.floor(n.task.progress * 100) / 100}%`
            );
         },
         popup_on: 'click',
         readonly_progress: !1,
   ...
   class S {
      ...
      change_view_mode(t = this.options.view_mode, e = !1) {
         ...
         e &&
            ...
            this.render(),
            e &&
               ((this.$container.scrollLeft = i), (this.options.scroll_to = s)),
      ...
      make_grid_ticks() {
         ...
         if (this.options.lines !== 'vertical')
            for (
               let h = this.config.header_height;
               h < this.grid_height;
               h += o
            )
               f('line', {
         ...
         if (this.options.lines !== 'horizontal')
            for (let h of this.dates) {
               ...
               this.config.view_mode.thick_line &&
                  ...
                  this.view_is('month')
                     ? (t +=
                          (d.get_days_in_month(h) * this.config.column_width) /
                          30)
                     : this.view_is('year')
                     ? (t +=
                          (d.get_days_in_year(h) * this.config.column_width) /
                          365)
      ...
      set_scroll_position(t) {
         ...
         (this.current_date = d.add(
            this.gantt_start,
            (this.$container.scrollLeft + r.clientWidth) /
               this.config.column_width,
            this.config.unit
         )),
      ...
      bind_bar_progress() {
         ...
         p.on(this.$svg, 'mousemove', (o) => {
            ...
            if (h > t) {
               ...
               for (; g; )
                  (h = g[1]), (g = a.find(([c, u]) => h >= c && h < u));
            } else {
               let g = a.find(([c, u]) => h > c && h <= u);
               for (; g; )
                  (h = g[0]), (g = a.find(([c, u]) => h > c && h <= u));
            }
            let _ = h - t;
      ...
      clear() {
         ...
         (this.$svg.innerHTML = ''),
            ...
            (h = (o = this.$extras) == null ? void 0 : o.remove) == null ||
               h.call(o),
            (_ = (l = this.popup) == null ? void 0 : l.hide) == null ||
               _.call(l);
      }
   }
   S.VIEW_MODE = {
